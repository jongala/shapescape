...
var COLORMODES = ['normal', 'duo', 'trio'];
...
function weave(options) {
    ...
    var COMMONCOLORS = void 0;
    if (opts.commonColors === 'auto') {
        COMMONCOLORS = (0, _utils.randItem)([true, false]);
    } else {
        COMMONCOLORS = (0, _utils.friendlyBoolean)(opts.commonColors);
    }
    ...
    if (COLORMODE === 'duo') {
        contrastPalette = (0, _utils.shuffle)(contrastPalette).slice(0, 2);
    }
    if (COLORMODE === 'trio') {
        contrastPalette = (0, _utils.shuffle)(contrastPalette).slice(0, 3);
    }
    ...
    // define grid
    var count = Math.round((0, _utils.randomInRange)(15, 30));
    var w = Math.ceil(cw / count);
    var h = w;
    var vcount = Math.ceil(ch / h);
    ...
    // weave params
    var perCell = (0, _utils.randomInt)(5, 10);
    var spacing = w / perCell;
    var openness = (0, _utils.randomInRange)(0.33, 0.66);
    var weight = spacing * openness;
    ...
    var drawStroke = function drawStroke(stroke) {
        ...
        ctx.beginPath();
        ctx.moveTo.apply(ctx, _toConsumableArray(stroke.start));
        ctx.lineTo.apply(ctx, _toConsumableArray(stroke.end));
        ctx.stroke();
    ...
    var warpColor = void 0;
    var weftColor = void 0;

    var warpPalette = [].concat(contrastPalette);
    var weftPalette = [].concat(contrastPalette);
    ...
    for (var i = 0; i < vcount; i++) {
        ...
        // delete half
        var cull = (0, _utils.randomInt)(warps.length / 2);
        while (cull--) {
            warps.splice((0, _utils.randomInt)(warps.length - 1), 1);
        }

        // Combine and shuffle the warp and weft arrays into strokes
        // so they can overlap in a mixed way
        strokes = (0, _utils.shuffle)([].concat(warps).concat(wefts));
